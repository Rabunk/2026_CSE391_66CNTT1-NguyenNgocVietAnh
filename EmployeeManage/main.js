document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (name === '') {
      nameInput.setCustomValidity('Họ tên không được để trống');
      nameInput.reportValidity();
      return;
    }
    if (name.length > 30) {
      nameInput.setCustomValidity('Họ tên tối đa 30 ký tự');
      nameInput.reportValidity();
      return;
    }
    if (email === '') {
    ...
    const newId = EmployeeData.length ? Math.max(...EmployeeData.map(x => x.id)) + 1 : 1;
  ...
  employeeTableBody.addEventListener('click', (e) => {
    if (e.target.matches('.btn-delete')) {
      ...
      if (idx > -1 && confirm('Bạn có chắc muốn xoá nhân sự này?')) {
    ...
    if (e.target.matches('.btn-edit')) {
      ...
      form.onsubmit = function (ev) {
        ev.preventDefault();
        // reuse validation above
        const updatedName = nameInput.value.trim();
        const updatedEmail = emailInput.value.trim();
        const updatedPhone = phoneInput.value.trim();
        const updatedPosition = positionInput.value;
        if (updatedName === '' || updatedName.length > 30) {
          nameInput.setCustomValidity('Họ tên không được để trống và tối đa 30 ký tự');
          nameInput.reportValidity();
          return;
        }
        if (updatedEmail === '') {
        ...
        renderTable(EmployeeData);
        employeeModal.hide();
        form.onsubmit = null; // restore default handler
      };
    }
  });
});
